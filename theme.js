...
function SidebarMouseHoverExpandButton() {
    ...
    if (getItem("isSidebarHover")) _sidebarHoverButtonImplementEven();
}
...
function sidebarHoverButtonImplementEven() {
    ...
    if (flag98 == 1 && openPanel()) {
        ...
        removeItem("sidebarRightWidth");
        removeItem("sidebarLeftWidth");
        ...
        setItem("isSidebarHover", true);
        setItem("sidebarRightWidth", sidebarRightWidth);
        setItem("sidebarLeftWidth", sidebarLeftWidth);
    }
}
...
function openLeftPanel() {
    if (getItem("sidebarLeftWidth")) {
        sidebarLeft.style.width = getItem("sidebarLeftWidth");
    } else {
        sidebarLeft.style.width = "400px";
    }
    sidebarLeft.style.position = "static";
    sidebarLeft.style.removeProperty('z-Index');
    leftHoverBlock_open();
}
function leftHoverBlock_open() {
    ...
    LeftHoverBlock.style.left = (parseInt(getItem("sidebarLeftWidth")) + 24) + "px";
    LeftHoverBlock.style.top = "5%";
    LeftHoverBlock.style.height = "90%";
...
function openRightPanel() {
    if (getItem("sidebarRightWidth")) {
        sidebarRight.style.width = getItem("sidebarRightWidth");
    } else {
        sidebarRight.style.width = "400px";
    }
    sidebarRight.style.position = "static";
    sidebarRight.style.removeProperty('z-Index');
...
function rightHoverBlock_open() {
    ...
    RightHoverBlock.style.right = (parseInt(getItem("sidebarRightWidth")) + 24) + "px";
    RightHoverBlock.style.top = "5%";
    RightHoverBlock.style.height = "90%";
    RightHoverBlock.style.zIndex = "9999";
    AddEvent(RightHoverBlock, "mouseover", (e) => {
...
function init() {
    ...
    // 历史条目保存到该数组
    let historyArr = [];
    if (getItem("historyArr")) {
        historyArr = JSON.parse(getItem("historyArr"));
    }
    // 新打开页签后更新历史记录
    function update_history_tags(newTag) {
        ...
        setTimeout(() => {
            ...
            if (current_doc) {
                ...
                setItem("historyArr", JSON.stringify(historyArr));
            }
        }, 700);
    }
    ...
    function showAllHistoryItems(e) {
        ...
        if (
            getItem("historyArr") &&
            JSON.parse(getItem("historyArr")).length > 0
        ) {
            historyArr = JSON.parse(getItem("historyArr"));
...
function SetItem(key, value, fun = null) {
    获取文件('/data/widgets/HBuilderX-Light-Custom.json', (v) => {
        if (v) {
            v[key] = value;
            console.log(v);
            写入文件("/data/widgets/HBuilderX-Light-Custom.json", JSON.stringify(v, undefined, 4), fun);
        }
    });
}
...
获取文件("/data/widgets/HBuilderX-Light.config.json", (v) => {
    let funs = () => {
    ...
    if (v == null) {
        window.HBuilderXLight.config = { "HBuilderXLight": 1 };
        写入文件("/data/widgets/HBuilderX-Light.config.json", JSON.stringify(window.HBuilderXLight.config, undefined, 4), (a) => { funs() });
    } else {
        window.HBuilderXLight.config = v;
        funs();
    }
    setInterval(() => {
        写入文件("/data/widgets/HBuilderX-Light.config.json", JSON.stringify(window.HBuilderXLight.config, undefined, 4));
    }, 5000)
});
